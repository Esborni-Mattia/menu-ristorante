...
if (isset($_GET['toggle']) && is_numeric($_GET['toggle'])) {
    try {
        ...
        $sqlToggle = "
            UPDATE prodotto
            SET disponibile = IF(disponibile = 1, 0, 1)
...
try {
    ...
    // Lettura dati con JOIN per avere il nome della categoria
    $sql = '
        SELECT p.id_prodotto, p.nome, p.prezzo, p.disponibile, c.nome AS categoria
        FROM prodotto p
            ...
            <tbody>
                <?php foreach ($ris as $r): ?>
                    <tr>
                        ...
                        <td class="prezzo-cella">€ <?= number_format($r['prezzo'], 2, ',', '.') ?></td>
                        <td><?= htmlspecialchars($r['categoria']) ?></td>
                        <td class="w3-center" style="white-space: nowrap;">
                            <a href="modificaedit.php?id=<?= $r['id_prodotto'] ?>" class="w3-button w3-tiny w3-blue w3-round azione-btn">
                                <i class="fa fa-pencil"></i>
                            </a>
                            <a href="cancellaconferma.php?id=<?= $r['id_prodotto'] ?>" class="w3-button w3-tiny w3-red w3-round azione-btn">
                                <i class="fa fa-trash"></i>
                            </a>

                             <a href="index.php?toggle=<?= $r['id_prodotto'] ?>"
                                class="w3-button w3-tiny <?= $r['disponibile'] ? 'w3-green' : 'w3-grey' ?> w3-round azione-btn"
                                title="<?= $r['disponibile'] ? 'Nascondi dal menu' : 'Rendi disponibile' ?>">
